...
//	전체 주문 취소: []
	public static void main(String[] args) {
		Set<Food> orderList = new HashSet<Food>();
		
		orderList.add(new Food("샐러드", 14000));
		orderList.add(new Food("치킨", 23000));
		orderList.add(new Food("치킨", 50000));
		
		System.out.println(orderList);
		
		Iterator<Food> iterator = orderList.iterator();
		
		while(iterator.hasNext()) {
			Food element = iterator.next();
			
			if (new Food("치킨", 50000).equals(element)) {
				iterator.remove();
			}
		}
		
		// orderList.remove(new Food("치킨", 50000)); 위의 코드와 동일
		
		System.out.println("오만원 치킨 취소: " + orderList);
