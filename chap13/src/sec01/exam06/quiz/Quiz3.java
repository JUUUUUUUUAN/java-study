...
//	키: Mercury, 값: 수성(지름: 4879km, 공전주기: 0.24년)
	...
	public static void main(String[] args) {
		...
		for (String key : keySet) {
			Planet planet1 = planet.get(key);
			String name = planet1.getName();
			int diameter = planet1.getDiameter();
			double period = planet1.getPeriod();
			System.out.println("행성이름: " + key + "(" + name + "), 지름: " + diameter + "km, 공전주기: " + period + "년");
		}
		System.out.println();
		
		Set<Entry<String, Planet>> entrySet = planet.entrySet();
		for (Entry<String, Planet> entry : entrySet) {
			String key = entry.getKey();
			Planet planet1 = entry.getValue();
			System.out.println("키: " + key + planet1.toString());
		}
	}
}
