...
//		List<String> keySet1 = new ArrayList<String>(keySet); => 이 경우는 Hash일 경우 사용
		for (String key : keySet) {
			int value = map.get(key);
			System.out.println("\t" + key + " : " + value);
//			map.remove(key); // 예외 발생 -> Iterator 사용 시 안전
		}
		System.out.println();
...
//			map.remove(key); // 테스트: 예외 발생 -> Iterator 사용 시 안전
		...
		// 찾는 값이 있으면 첫 번째로 발견되는 즉시 true를 반환(중복이 있더라도 첫 번재 매치에서 멈춤)
		int valueToCheck = 80;
